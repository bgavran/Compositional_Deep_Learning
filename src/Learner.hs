...
--
--
---- applies a function to the codomain of the input DType
--appliedFn :: (Additive4 a b c x)
--          => ((a,  b) -> c) -> DType x (a, b) -> DType x c
--appliedFn fn = (linearD fn .)
--
...
--
--  id = Para exr
--  (Para g) . (Para f) = Para $ let fn = appliedFn (uncurry (.))
--                               in curryUncurry2 (tupleToZ . fn) g f
--
...
--
--  (Para f) `x` (Para g) = Para $ let fn = appliedFn (uncurry x)
--                                 in curryUncurry2 (tupleToZ . fn) f g
--
--------------------------------------
--
--type ZF p = Z p
